...
6. **Run the spec check once, before that confirmation — whether the definition arrived as a file or you just wrote it.** Spawn one subagent with the spec-checker role contract, the task definition path, the convention doc paths and knowledge layer, and an output path such as `<loop dir>/spec-gaps.json`. **Wait for an answer only when step 5 wrote a brief**: then show its `gaps` on the same screen as that confirmation, so the decisions it surfaced get answered while a human is still there, and fold the answers back into the task definition file before the first cycle. When the definition arrived as a file there is no confirmation to attach to — print the `gaps` and start. Do not wait for an answer that has no screen to arrive on; an unattended run would stall silently rather than fail. **This is a warning layer, not a gate** — it never blocks the start. Which gaps are load-bearing differs per project, and a machine that blocks on them only teaches people to route around it. If the check itself fails to produce a file, say so and start anyway.
...
3. **Delegate the checker.** Spawn a separate subagent whose task is the checker role contract plus **the task definition file path from setup step 5** — the same path the maker was given, not a summary you write here — the compare base, and the findings output path. It writes `{"base", "reviewed": [...], "findings": [...]}` to that path — `reviewed` lists the changed files it actually read. Do not review the diff yourself and do not write the findings file yourself. Maker and checker working from different summaries of one task is the failure this shared path prevents: the intent dimension would be judging the diff against something the maker was never asked to build.
4. **Score (deterministic).** Run exactly `bash "$ENGINE/score.sh" <findings path> | bash "$ENGINE/decide.sh"`. It prints a JSON object with a `verdict` field. Use only what it prints. **Check the exit status.** Exit 65 means the scorer refused the input — it is not a verdict and there is nothing to obey. The common cause is a clean result whose `reviewed` list is empty, which the scorer cannot tell apart from a checker that never looked. Stop and hand to a human; do not treat an empty result as a pass.
5. **Obey the verdict.**
