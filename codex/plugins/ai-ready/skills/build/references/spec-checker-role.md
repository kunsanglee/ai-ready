...
For the decomposition file, the machine check already verified that `exit_criteria`, `irreversible`, `non_goals`, and `tiebreaks` are *present*. You judge whether they are *usable*: does each exit criterion name something that turns red when reverted (rather than "make X better", which never terminates), do the criteria together represent what the design asked of that phase, does `irreversible: false` hold given what the phase's steps touch, and do the tiebreaks cover the trade-offs visible in the design.

`non_goals` goes wrong in both directions, so check both. Too wide and work the phase genuinely has to do lands on the "not looking at this" list, where a correct implementation comes back as `intent-nongoal-violation` (BLOCKER) and the unattended run stops. Too narrow — or a `false` with no basis — and it filters nothing, leaving the field decorative. An entry overlapping the step list or `exit_criteria` is the first case; something the design says it will not do this round that is absent from `non_goals` is the second (lens `purpose`).
