...
> 무인 검증 loop 의 **앞단**. 호출: `/spec [초안·설계문서 경로]`. 산출물을 그대로 `/build` 에 넘긴다.

`/build` 는 스펙이 **이미 있다고 가정하고** phase 로 쪼갠다. 이 스킬은 그 앞에 서서 **스펙 자체를 만든다.** 대화에서 합의된 것, 초안 문서에 적힌 것, 기존 코드에 이미 정해진 것을 모아, 아직 아무도 안 정한 결정을 사람 앞에 놓고 하나씩 닫는다.

## 왜 이 층이 있나

**무인 완주를 가르는 것은 사람 게이트가 아니라 스펙의 질이다.** 같은 날 같은 저장소에서 돌린 두 phase 가 근거다. 목표를 "세웠다고 적은 장치가 실제로 잠기게 한다" 로 준 phase 는 여섯 사이클을 돌고 사람이 한 번 끼어들어야 닫혔다. 목표를 변이 여섯으로 미리 적어 준 phase 는 네 사이클에 사람 없이 닫혔다. 차이는 스펙 하나였다.

`/build` 의 착수 전 검사는 그 자리를 **막기만** 한다. 세 자리가 비면 "채우고 다시 와라" 로 돌려보낼 뿐, 무엇을 채워야 하는지는 사람이 혼자 알아내야 한다. 이 스킬이 그 "혼자" 를 없앤다.

## 절대 원칙

1. **답을 지어내지 않는다. 이것이 이 스킬의 전부다.** 스펙의 구멍은 대부분 **사람 머릿속에만** 있다. 거기서 "지적받았으니 보완해라" 를 시키면 작성하는 쪽이 그럴듯한 답을 채우고, 다음 회차의 점검이 "이제 정해졌네" 하고 통과시킨다. 그러면 게이트는 초록인데 사람 의도와 무관한 스펙이 나오는 **환각 수렴기**가 된다. 그래서 모든 결정에는 **처분과 근거**가 함께 붙고, 근거 없는 채움은 아래 기계 검사가 거부한다.
2. **종료 조건은 통과가 아니라 미결 0 이다.** "이만하면 됐다" 를 모델이 판정하지 않는다. 결정 원장에 `open` 이 하나도 없고 `asked` 에 전부 사람 답이 붙어 있을 때만 끝난다. 그 판정은 셸이 센다.
3. **도출과 질문은 이 세션이 한다.** 서브에이전트는 사람에게 직접 못 물어 왕복이 성립하지 않는다. 그래서 열거(점검)만 `loop-spec-checker` 에 내리고, 처분과 질문과 반영은 이 스킬 본문이 메인에서 진행한다.
...
5. **미결을 남기고 끝낼 수는 있되 조용히는 안 된다.** 사람이 "그건 나중에" 라고 하면 그 항목은 `deferred` 로 남기고 산출 문서에 **미결 절**로 적는다. `/build` 로 넘어간 뒤 그 자리에서 maker 가 추측으로 메우는 것이 이 스킬이 막으려던 바로 그것이라, 넘긴다는 사실을 문서가 들고 간다.

## 🔌 plugin / 프로젝트 구조

- 이 스킬은 `ai-ready` plugin 의 일부다. 의존(plugin 번들): `agents/loop-spec-checker.md`(결정 자리 열거, `ai-ready:` namespace), `_loop-engine/detect_build.py`(컨벤션 문서·지식층 경로 감지), `_loop-engine/lib.sh`.
- 산출(프로젝트 델타): `.loop/spec/{slug}/decisions.json`(결정 원장), `.loop/spec/{slug}/spec.md`(사람이 읽고 `/build` 가 받는 설계 문서). 사용자가 정본 위치를 지정하면 `spec.md` 는 그리로 옮긴다(예: `docs/design/`).
- 환경변수 없음. 경로는 런타임 감지와 이 스킬의 Step 0 이 파일로 남긴다.
...
`ai-ready:loop-spec-checker` 호출이 "Agent type not found" 로 죽는 일이 실제로 있다. **플러그인을 업데이트했더라도 이미 돌고 있는 세션은 시작 시점의 에이전트 목록을 그대로 쓰기 때문이다.** 파일은 `agents/loop-spec-checker.md` 에 멀쩡히 있고 매니페스트에도 등록돼 있는데 그 세션의 목록에만 없는 상태가 되고, 이 스킬이 처음 나가는 릴리스에서는 오히려 그 세션이 기본값이다.

**그때는 다른 에이전트로 대신 돌리지 않는다. 멈추고 사람에게 세션을 다시 시작하라고 말한다.**

대신 돌리는 길을 열어 두면 그 길이 곧 기본 경로가 된다. 열거가 이 스킬의 전부인데 그 열거를 누가 했는지가 실행마다 달라지면, 같은 스펙이 회차마다 다른 개수의 결정을 내고 무엇이 빠졌는지 아무도 설명하지 못한다. 이 저장소가 `/loop-run` 을 없앤 이유와 같다. **검사 옆에 우회로가 있으면 그 검사는 검사가 아니라 권고다.**

멈출 때 사람에게 주는 것은 세 줄이다.

1. 무엇이 없나 — `ai-ready:loop-spec-checker` 가 이 세션의 에이전트 목록에 없다.
2. 왜 그런가 — 목록은 세션 시작 시점에 고정된다. 설치는 정상일 수 있다.
3. 어떻게 고치나 — 세션을 다시 시작하고 `/spec` 을 다시 부른다. Step 0 이 만든 원장은 그대로 남아 있어 처음부터 다시 묻지 않는다.

원장이 남는다는 마지막 줄이 이 게이트를 감당할 만하게 만든다. 재시작 비용이 "지금까지 답한 것을 다시 답하는 것" 이면 사람은 우회로를 요구하게 된다.

Step 0 이 파일 실재를 미리 확인하므로, 여기까지 왔는데 호출이 죽었다면 원인은 설치가 아니라 세션이다. 파일이 아예 없는 경우는 Step 0 에서 이미 걸러진다.
...
**완료 조건 절이 `/build` 의 `exit_criteria` 가 되고, 안 만드는 것 절이 `non_goals` 의 재료가 되고, 미결 절이 `irreversible`·`tiebreaks` 판단 재료가 된다.** 그래서 이 절들을 대충 적으면 `/build` 의 착수 전 검사가 바로 그 자리에서 막는다. 안 만드는 것 절은 오래 `/build` 에 받을 칸이 없어 그 스킬까지 안 흘렀는데, 1.4.0 에서 `non_goals` 가 생기며 이어졌다.

**다만 알갱이가 다르다. 이 문서의 두 절은 문서에 하나씩이고 `/build` 의 두 칸은 phase 마다 하나다.** 그래서 옮길 때 나눠야 한다. 이 문서의 "안 만드는 것" 은 **작업 전체가 안 만드는 것**이라 모든 phase 의 `non_goals` 에 공통으로 들어가고, 거기에 **그 phase 만 안 보는 표면**(다음 phase 가 볼 것)을 더한다. 그 "다음 phase 가 본다" 는 이 문서에 없는 정보라 분해할 때 정해진다. 그대로 복사만 하면 phase 별 표면 좁히기가 하나도 안 생겨 `non_goals` 가 있으나 마나가 된다. `exit_criteria` 도 같은 방식으로 나뉜다.

이어서 `/build {산출 문서 경로}` 를 부르면 된다.

## 이 스킬이 하지 않는 것 (Non-Goals)

- **스펙을 대신 써 주는 것.** 이 스킬은 결정을 **열거하고 처분**한다. 사람이 답해야 할 자리를 대신 채우는 순간 원칙 1이 깨진다.
...
| 근거없음으로 exit 65 | 처분은 적었는데 evidence·answer·note 가 빈 값 | 그 항목의 근거를 채운다. 못 채우면 처분을 `open` 으로 되돌린다 |
| 미결 0 인데 `/build` 착수 전 검사에서 막힌다 | 완료 조건을 "~하게 만든다" 로 적었다 | 되돌림 문장으로 다시 쓴다("X 를 지우면 그 검사가 실패한다") |
