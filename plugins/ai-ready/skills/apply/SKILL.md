...
스크립트는 audit 스킬 폴더에 있다. 아래 명령의 `<AUDIT>` 는 이 스킬 본문 첫머리의 "Base directory for this skill"
절대 경로 끝의 `apply` 를 `audit` 으로 바꾼 경로, `<T>` 는 대상 저장소 절대 경로다. Bash 도구의 셸에는
`$CLAUDE_PLUGIN_ROOT` 가 없으니 두 경로를 글자 그대로 넣는다.

- 명령은 한 줄로 부른다. 변수 대입·`set -euo pipefail`·`{ ...; exit ...; }` 묶음을 앞에 붙이지 않는다 — 권한 검사가
  그런 명령을 멈춰 세운다. 스크립트가 있는지 먼저 보려면 `test -f <AUDIT>/scripts/bootstrap.py && python3 ...` 꼴까지만
  쓴다.
...
```bash
python3 <AUDIT>/scripts/bootstrap.py --target <T> --only root,design --dry-run
```

```bash
python3 <AUDIT>/scripts/bootstrap.py --target <T> --only root,design
```
...
- 만들 파일이 git 에서 무시되면(`.gitignore` 에 `CLAUDE.md` 가 있는 저장소 등) 아무것도 쓰지 않고 exit 6 으로 끝나며
  무시 규칙의 위치(`.gitignore:43` 꼴)를 출력한다. 무시 규칙을 고칠지 사람에게 묻고, `--force` 로 넘기지 않는다.
...
```bash
python3 <AUDIT>/scripts/scaffold.py --target <T> --out <T> --modules app,core
```

- 두 번째 꼴은 모듈을 직접 고를 때 쓴다. 모듈마다 `AGENTS.md`(초안)와 `@AGENTS.md` 한 줄짜리 `CLAUDE.md` 를 만든다.
  사람이 관리하는 문서나 심볼릭 링크가 있는 모듈을 직접 고르면 exit 3, 만들 파일이 git 에서 무시되면 exit 6 이다.
- 절: 이 모듈이 하는 일 / 경계(무엇에 의존하고 무엇이 의존하나) / 변경 방법 / 강제할 수 없는 규칙(항목마다 왜) /
  강제되는 규칙("→ <lint 규칙·테스트>" 포인터만).
- 사람이 관리하는 `AGENTS.md`·`CLAUDE.md` 가 이미 있거나 심볼릭 링크인 모듈은 후보에서 뺀다. 모든 모듈에 한꺼번에
  만들지 않는다. 최근 변경이 잦은 모듈부터 몇 개만 만들고, 채운 뒤에 넓힌다.
...
- **TODO 를 채울 때는 `Write` 가 아니라 `Edit` 로 TODO 자리만 고친다.** 첫 줄의 서명
  (`<!-- ai-ready:apply 자동 생성 초안 ...`)은 기본으로 남긴다. 남겨 두면 다음 apply 가 이 파일을 초안으로 보고 다시
  만들 수 있다. 지우면 사람이 관리하는 파일이 되어 ai-ready 가 더는 덮어쓰지 않는다. 지울지는 사람이 정한다.
- TODO 채우기를 하위 에이전트에 넘길 때도 이 두 조건(`Edit` 로 고친다, 서명 줄은 남긴다)을 프롬프트에 그대로 적는다.
