...
- **핵심 성질**
  ...
  - checker 는 축이 갈린 렌즈 셋으로 **병렬**로 돌고, 결과는 **개수를 세어** 합칩니다. 렌즈 하나가 죽어도 남은 것의 형식은 멀쩡해서, 세지 않으면 그 축이 한 번도 점검되지 않은 채 통과합니다. 점검 파일이 많은 회차는 safety·quality 를 샤드로 더 나눠 병렬을 늘립니다(`contract` 는 항상 전체 — 파일 사이의 계약 결함을 보는 시야는 남긴다). 샤딩 회차의 PASS 는 비샤딩 확인 회차를 한 번 거칩니다.
