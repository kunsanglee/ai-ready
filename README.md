...
| **샤드** | 한 렌즈가 읽을 파일 목록을 결정론으로 나눈 조각. 점검 파일이 많으면 `safety`·`quality` 를 샤드마다 한 명씩 띄워 병렬을 늘립니다. `contract` 는 파일 사이를 봐야 해서 안 나눕니다. |
...
- **동작** — Step 0 셋업 → Step 1 분해 → **착수 전 스펙 검사**(기계) → **스펙 완전성 점검**(`loop-spec-checker`) → **사람 승인(유일한 시작 게이트)** → Step 1-1 순회를 서브에이전트에 위임 → Step 2 phase 순회(빌드 게이트 → checker 렌즈 셋 병렬, 점검 파일이 많으면 샤드로 더 나눔 → 병합·채점 → 분기) → Step 3 완료 보고.
...
- **핵심 성질**
  ...
  - checker 는 축이 갈린 렌즈 셋으로 **병렬**로 돌고, 결과는 **개수를 세어** 합칩니다. 렌즈 하나가 죽어도 남은 것의 형식은 멀쩡해서, 세지 않으면 그 축이 한 번도 점검되지 않은 채 통과합니다. 점검 파일이 많은 회차는 safety·quality 를 샤드로 더 나눠 병렬을 늘립니다(`contract` 는 항상 전체 — 파일 사이의 계약 결함을 보는 시야는 남깁니다). 샤딩 회차의 PASS 는 비샤딩 확인 회차를 한 번 거칩니다.
  - 회차·시간·예산 상한을 **phase 하나 기준**으로 봅니다. 전체 시간 상한은 `phase당 120분 × phase수` 로 자동 확장돼, phase 가 늘어도 뒤 phase 가 시간에 잘리지 않습니다.
  - 빌드·테스트·품질 검사 명령, 티켓 패턴, 베이스 브랜치는 `detect_build.py` 가 런타임에 감지해 어댑터 파일을 남기지 않습니다.
  - 게이트가 깨지면 그 출력은 창이 아니라 파일로 가고, `gate_parse.py` 가 항목 큐(`.loop/run/{ticket}/gate-queue.jsonl`)로 바꿉니다. maker 는 컴파일 오류 → 테스트 실패 → 린트·복잡도 위반 순으로 항목을 꺼내 고칩니다.
...
| `detect_build.py` | 매니페스트·브랜치를 읽어 빌드·테스트·품질 검사 명령, 티켓 패턴, 베이스 브랜치, 컨벤션 문서를 런타임 감지 |
| `gate_parse.py` | 게이트(빌드·테스트·품질) 실패 출력을 한 줄 하나의 JSON 항목으로 바꿈. maker 가 항목 단위로 꺼내 고친다 |
